...
export class YoutubeStuff {
	...
	// Function to search for a channel by name, return channel channels (items) as array
	async searchChannel(apiKey, channelName) {
		try {
			const response = await fetch(
				`https://www.googleapis.com/youtube/v3/search?part=snippet&type=channel&q=${encodeURIComponent(channelName)}&key=${apiKey}`
			);
			// NOTE: returns valid via: return.items[i].id.channelId
	...
	// Function to get channel ID from channel data 
	// NOTE: AKA, Select from list
	getChannelId(channelData) {
		return channelData.id.channelId;
	}

	// Function to get live broadcasts for a channel
	async getChannelBroadcasts(apiKey, channelId) {
		try {
			const response = await fetch(
	...
	// Function to get live broadcast ID from broadcast data
	getLiveBroadcastId(broadcastData) {
		return broadcastData[0].id.videoId;
	}

	// Example usage of all functions together
	async getAllBroadcastInfo(apiKey, channelName) {
		try {
			// Search for channel
			const channelData = await searchChannel(apiKey, channelName);

			// Get channel ID
			const channelId = getChannelId(channelData);

			// Get broadcasts
			const broadcasts = await getChannelBroadcasts(apiKey, channelId);

			// Get live broadcast ID
			const broadcastId = getLiveBroadcastId(broadcasts);
			...
			console.error('Error in broadcast info workflow:', error);
			throw error;
		}
	}

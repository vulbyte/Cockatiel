...
# Julian Date Compatibility Methods for Chrono

This library provides compatibility with astronomical applications that use Julian Dates as 64-bit floats. A *Julian Date* (JD) is the Julian Day Number plus the fraction of a day elapsed since the preceding noon UT, referenced from the start of the Julian period at 12 noon on November 24, 4713 BCE (-4713-11-24T12:00:00 UTC). Julian Dates facilitate calculations over extended periods and should not be confused with the Julian Calendar, which affects leap year rules.
